...
class Shared2FCInstanceMILHeadENoiseBox(Shared2FCBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(Shared2FCInstanceMILHeadENoiseBox, self).__init__(
        ...
        self.dyna_channels = 1
        self.param_linear = nn.Linear(
            self.fc_out_channels, (self.in_channels + 1), )
    ...
    def forward(self, x, stage,others=None):
        ...
        if others:
            proposal_feat = others['x_feat']
            proposal_feat = torch.cat(proposal_feat)
            num_gt = proposal_feat.shape[0]
            param = self.param_linear(proposal_feat).reshape(-1, self.in_channels + 1)
            weight = param[:, :self.in_channels]
            bias = param[:, self.in_channels:].squeeze(-1)
            weight1 = weight.reshape(-1, self.in_channels, 1, 1)
            bias1 = bias.reshape(-1)
            x_ = F.conv2d(x, weight1, bias=bias1, stride=1, padding=0, dilation=1, groups=1)
            x_ = x_.reshape(num_gt, -1, num_gt, *x.shape[-2:])
            affinity = x_.softmax(dim=2)
            flag = affinity.new_full((affinity.shape), -1, dtype=torch.long)
            flag[torch.arange(num_gt), :, torch.arange(num_gt), ...] = 1
            affinity = (affinity * flag).reshape(x.shape[0], -1, *x.shape[-2:])
            feat = x.reshape(num_gt, -1, *x.shape[-3:]).sum(1)
            affinity = affinity.reshape(x.shape[0], num_gt, -1).permute(2, 0, 1)
            feat = feat.reshape(num_gt, x.shape[1], -1).permute(2, 0, 1)
            new_feat = torch.bmm(affinity, feat).permute(1, 2, 0).reshape(x.shape)
            new_feat = x + new_feat
        # shared part

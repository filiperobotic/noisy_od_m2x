...
class EP2BHead(StandardRoIHead):
    """Simplest base roi head including one bbox head and one mask head."""

    def __init__(self, bbox_roi_extractor, num_stages, bbox_head, bbox_head1=None, stage_modes=['CBP', 'PBR'], top_k=7,
                 with_atten=None, **kwargs):
        super(EP2BHead, self).__init__(bbox_roi_extractor=bbox_roi_extractor, bbox_head=bbox_head, **kwargs)
        self.threshold = 0.3
    ...
    def forward_train(self,
        ...
        if self.with_atten:
            x = self.atten_pts_and_ftmap(x, gt_true_bboxes, img_metas)
        losses = dict()
    ...
    def _bbox_forward(self, x, rois, gt_points, stage, others=None):
        ...
        if gt_points is not None:
            ...
            bbox_results = dict(
                cls_score=cls_score, ins_score=ins_score, bbox_pred=reg_box, bbox_feats=bbox_feats, num_instance=num_gt,others=others)
            return bbox_results
        # megative sample
        else:
            bbox_results = dict(
    ...
    def merge_box_single(self, cls_score, ins_score, dynamic_weight, gt_point, gt_label, proposals, img_metas, stage):
        ...
        # ##vote
        if merge_mode == 'vote':
            dynamic_weight_, idx = dynamic_weight.topk(k=10, dim=1)
            ...
            boxes1 = (filtered_boxes * weight).sum(dim=1)

            grid_box = filtered_boxes.round().long()
        ...
        if merge_mode == 'weighted_cls':
            ...
            boxes = (proposals * weight).sum(dim=1)
        ...
        if merge_mode == 'weighted_cls_topk':
            ...
            # print(boxes)
            return boxes, None, None
        if merge_mode == 'cls_mix_rules':
            # choose best shape
            cls = cls_score.reshape(-1, 9, 5)
            _, idx = cls[..., 0].topk(3)
            proposals_old = proposals
            proposals = proposals.reshape(-1, 9, 5, 4)[torch.arange(len(proposals)).unsqueeze(-1), idx]
            cls = cls[torch.arange(len(proposals)).unsqueeze(-1), idx]

            # choose lr td
            lr = cls.mean(dim=1)[:, 1:3].max(dim=-1)
            td = cls.mean(dim=1)[:, 3:5].max(dim=-1)

            base_cls = cls[:, :, 0]
            base_pps = proposals[:, :, 0]
            base_lr_cls = cls[:, :, 1:3][torch.arange(len(proposals)), :, lr[-1]]
            base_lr_pps = proposals[:, :, 1:3][torch.arange(len(proposals)), :, lr[-1]]
            base_td_cls = cls[:, :, 1:3][torch.arange(len(proposals)), :, td[-1]]
            base_td_pps = proposals[:, :, 3:5][torch.arange(len(proposals)), :, td[-1]]

            lr_cls = torch.cat([base_cls, base_lr_cls], dim=1)
            td_cls = torch.cat([base_cls, base_td_cls], dim=1)
            lr_pps = torch.cat([base_pps, base_lr_pps], dim=1)
            td_pps = torch.cat([base_pps, base_td_pps], dim=1)

            # cal weight for merge
            lr_weights = lr_cls / (lr_cls.sum(dim=-1, keepdim=True) + 1e-8)
            td_weights = td_cls / (td_cls.sum(dim=-1, keepdim=True) + 1e-8)
            lr_pseudo_box = (lr_pps * lr_weights.unsqueeze(-1)).sum(dim=1)
            td_pseudo_box = (td_pps * td_weights.unsqueeze(-1)).sum(dim=1)
            pseudo_box = torch.cat([lr_pseudo_box[..., 0:4:2], td_pseudo_box[..., 1:4:2]], dim=-1)

            # xxyy tp xyxy
            pseudo_box = pseudo_box[:, [0, 2, 1, 3]]
        ...
        if merge_mode == 'max_score':
            ...
            pseudo_box = proposals[torch.arange(proposals.shape[0]), idx]
    ...
    def merge_box(self, bbox_results, proposals_list, proposals_valid_list, gt_labels, gt_true_bboxes, img_metas,
                  stage):
        ...
        # split batch
        batch_gt = [len(b) for b in gt_true_bboxes]
        ...
        # xy_box = bbox_xyxy_to_cxcywh(pseudo_boxes)
        # xy_gt = bbox_xyxy_to_cxcywh(torch.cat(gt_bboxes))
        # torch.abs(xy_gt[:,0]-xy_box[:,0])/xy_box[:,w]

        ### scale mean iou
        iou1 = bbox_overlaps(pseudo_boxes, torch.cat(gt_true_bboxes), is_aligned=True)
        gt_xywh = bbox_xyxy_to_cxcywh(torch.cat(gt_true_bboxes))
        ...
        # mean_iou_s=iou1[scale<8**2].mean()
        # mean_iou_m= iou1[(scale > 8 ** 2)*(scale < 16 ** 2)].mean()
        # mean_iou_l = iou1[(scale > 16 ** 2) * (scale < 32 ** 2)].mean()
        # mean_iou_h = iou1[scale > 32 ** 2].mean()
